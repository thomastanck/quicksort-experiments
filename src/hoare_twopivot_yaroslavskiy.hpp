...
inline std::tuple<RandomIt, RandomIt, RandomIt, RandomIt> twopivot_yaroslavskiy_partition_impl(RandomIt first, RandomIt last, RandomIt tmp_pivot_low, RandomIt tmp_pivot_high, Compare&& comp) {
    assert(first < last);
    --last;
    ...
    if (last != first) {
        ...
        while (true) {
            while (true) {
                ...
                if (!comp(*first, *pivot_high)) break;
                ++first;
            }
            // *first should go into the third partition
...
inline void twopivot_yaroslavskiy_quicksort_impl(RandomIt first, RandomIt last, Compare&& comp, BiPivotSelector&& bipivot_selector) {
    while (last - first > static_cast<std::ptrdiff_t>(InsertionSortThreshold)) {
        ...
        // partition the array
        auto [low_end, mid_begin, mid_end, high_begin] = twopivot_yaroslavskiy_partition_impl(first, last, pivot_low, pivot_high, comp);

        // recurse on all sub-ranges
        twopivot_yaroslavskiy_quicksort_impl<InsertionSortThreshold>(high_begin, last, comp, bipivot_selector);
        twopivot_yaroslavskiy_quicksort_impl<InsertionSortThreshold>(mid_begin, mid_end, comp, bipivot_selector);
        last = low_end;
    }
}

template <size_t InsertionSortThreshold, typename BiPivotSelector>
class twopivot_yaroslavskiy_quicksort {
private:
    BiPivotSelector bipivot_selector;

public:
    twopivot_yaroslavskiy_quicksort(BiPivotSelector bipivot_selector)
    ...
    void operator()(RandomIt first, RandomIt last, Compare comp) {
        ...
        if (dist >= 2) {
            twopivot_yaroslavskiy_quicksort_impl<InsertionSortThreshold>(first, last, comp, std::move(bipivot_selector));
...
template <size_t InsertionSortThreshold, typename BiPivotSelector>
twopivot_yaroslavskiy_quicksort<InsertionSortThreshold, BiPivotSelector> make_twopivot_yaroslavskiy_quicksort(
    BiPivotSelector bipivot_selector) {
    return twopivot_yaroslavskiy_quicksort<InsertionSortThreshold, BiPivotSelector>{std::move(bipivot_selector)};
}
