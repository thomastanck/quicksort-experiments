...
inline void hoare_quicksort_impl(RandomIt first, RandomIt last, Compare&& comp, PivotSelector&& pivot_selector) {
    while (last - first > static_cast<std::ptrdiff_t>(INSERTION_SORT_THRESHOLD)) {
        // pick a pivot
...
template <typename RandomIt, typename Compare, typename PivotSelector>
inline void hoare_quicksort(RandomIt first, RandomIt last, Compare&& comp, PivotSelector&& pivot_selector) {
